...
function bookmarks_print_item($item, $cmid, $userids='', $commands=false, $options=''){
	...
	echo '<p class="bookmark">
		<h3 class="name"><a class="previewlink" title="'.$link->url.'" href="link.php?id='.$cmid.'&url='.urlencode($link->url).'">'.$item->name.'</a></h3>';
	
	if ($commands){
		echo '<div class="commands">
				<a class="edit" href="edit.php?id='.$cmid.'&item='.$item->id.'">edit</a>
				/
				<a class="rm" href="del.php?id='.$cmid.'&item='.$item->id.'">delete</a>
			</div>';
	}
	...
	if ($options == 'detailed'){
		...
		echo 'Saved by '. $usr->lastname.', '. $usr->firstname;
	} else {
		...
		if ($countall>1){
			echo 'Saved by '.$countall.'(<a href="view.php?id='.$cmid.'&action=search&query='.urlencode('url:'.$link->url).'&options=detailed">'.$countpart.'</a>) people';
		} else{
			echo 'Saved by '.$countall.'(<a href="view.php?id='.$cmid.'&action=search&query='.urlencode('url:'.$link->url).'&options=detailed">'.$countpart.'</a>) person';
		}
		echo ' ... Seen '.$link->hits .' times';
